...
/// <summary>
/// Implements the DMAX function.
/// DMAX(database, field, criteria) - Maximum value in field that meets criteria.
...
/// </summary>
public sealed class DMaxFunction : IFunctionImplementation
{
    /// <summary>
    /// Gets the singleton instance.
    /// </summary>
    public static readonly DMaxFunction Instance = new();

    private DMaxFunction()
    {
    }

    /// <inheritdoc/>
    public string Name => "DMAX";

    /// <inheritdoc/>
    public CellValue Execute(CellContext context, CellValue[] args)
    {
        // DMAX requires exactly 3 arguments: database, field, criteria
        if (args.Length != 3)
        {
        ...
        if (MatchesCriteria(database, criteria))
        {
            ...
            {
                max = System.Math.Max(max, database.NumericValue);
                hasValue = true;
            }
        }
