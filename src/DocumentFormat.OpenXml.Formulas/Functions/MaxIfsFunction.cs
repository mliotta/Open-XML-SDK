...
/// <summary>
/// Implements the MAXIFS function.
/// MAXIFS(max_range, criteria_range1, criteria1, [criteria_range2, criteria2], ...) - Maximum of cells meeting multiple criteria.
/// </summary>
public sealed class MaxIfsFunction : IFunctionImplementation
{
    /// <summary>
    /// Gets the singleton instance.
    /// </summary>
    public static readonly MaxIfsFunction Instance = new();

    private MaxIfsFunction()
    {
    }

    /// <inheritdoc/>
    public string Name => "MAXIFS";

    /// <inheritdoc/>
    public FormulaResult Execute(CellContext context, FormulaResult[] args)
    {
        // MAXIFS requires at least 3 arguments (max_range, criteria_range1, criteria1)
        // and additional criteria pairs (criteria_range, criteria)
        // Total arguments must be odd (max_range + pairs of criteria_range/criteria)
        ...
        // If all criteria are met, include in max calculation
        if (allCriteriaMet)
        {
            if (maxRange.Type == FormulaResultType.Number)
            {
                max = System.Math.Max(max, maxRange.NumericValue);
                hasValue = true;
            }
        }
