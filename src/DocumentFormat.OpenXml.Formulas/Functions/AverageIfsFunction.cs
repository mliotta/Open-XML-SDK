...
/// <summary>
/// Implements the AVERAGEIFS function.
/// AVERAGEIFS(average_range, criteria_range1, criteria1, [criteria_range2, criteria2], ...) - Average of cells meeting multiple criteria.
/// </summary>
public sealed class AverageIfsFunction : IFunctionImplementation
{
    /// <summary>
    /// Gets the singleton instance.
    /// </summary>
    public static readonly AverageIfsFunction Instance = new();

    private AverageIfsFunction()
    {
    }

    /// <inheritdoc/>
    public string Name => "AVERAGEIFS";

    /// <inheritdoc/>
    public FormulaResult Execute(CellContext context, FormulaResult[] args)
    {
        // AVERAGEIFS requires at least 3 arguments (average_range, criteria_range1, criteria1)
        // and additional criteria pairs (criteria_range, criteria)
        // Total arguments must be odd (average_range + pairs of criteria_range/criteria)
        ...
        // If all criteria are met, include in average calculation
        if (allCriteriaMet)
        {
            if (averageRange.Type == FormulaResultType.Number)
            {
