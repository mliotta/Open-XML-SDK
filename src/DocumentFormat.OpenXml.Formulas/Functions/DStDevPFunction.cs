...
/// <summary>
/// Implements the DSTDEVP function.
/// DSTDEVP(database, field, criteria) - Population standard deviation of values in field that meet criteria.
...
/// </summary>
public sealed class DStDevPFunction : IFunctionImplementation
{
    /// <summary>
    /// Gets the singleton instance.
    /// </summary>
    public static readonly DStDevPFunction Instance = new();

    private DStDevPFunction()
    {
    }

    /// <inheritdoc/>
    public string Name => "DSTDEVP";

    /// <inheritdoc/>
    public FormulaResult Execute(CellContext context, FormulaResult[] args)
    {
        // DSTDEVP requires exactly 3 arguments: database, field, criteria
        if (args.Length != 3)
        {
        ...
        // DSTDEVP requires at least 1 value
        if (values.Count == 0)
        {
            return FormulaResult.Error("#DIV/0!");
        }

        // For a single value, population standard deviation is 0
        if (values.Count == 1)
        {
        ...
        return FormulaResult.FromNumber(stdDev);
    }
