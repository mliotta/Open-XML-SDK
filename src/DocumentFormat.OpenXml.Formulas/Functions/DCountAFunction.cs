...
/// <summary>
/// Implements the DCOUNTA function.
/// DCOUNTA(database, field, criteria) - Counts non-empty values in field that meet criteria.
...
/// </summary>
public sealed class DCountAFunction : IFunctionImplementation
{
    /// <summary>
    /// Gets the singleton instance.
    /// </summary>
    public static readonly DCountAFunction Instance = new();

    private DCountAFunction()
    {
    }

    /// <inheritdoc/>
    public string Name => "DCOUNTA";

    /// <inheritdoc/>
    public FormulaResult Execute(CellContext context, FormulaResult[] args)
    {
        // DCOUNTA requires exactly 3 arguments: database, field, criteria
        if (args.Length != 3)
        {
        ...
        // Apply criteria matching logic and count any non-empty value
        if (MatchesCriteria(database, criteria))
        {
            if (database.Type != FormulaResultType.Empty)
            {
                count++;
            }
        }
